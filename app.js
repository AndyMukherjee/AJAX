...
// this function takes the question object returned by StackOverflow 
// and creates new result to be appended to DOM
var showAnswere = function(answere) {
	
	// clone our result template code
	var result = $('.templates .Answere').clone();
	
	// Set the question properties in result
	var qElem = result.find('.q-text a');
	qElem.attr('href', q.link);
	qElem.text(q.title);
	//set the answere properties in result
	var answereElem = result.find('.answere-text a');
	answereElem.attr('href', answere.link);
	answereElem.text(answere.title);
...
// set the date replied property in result
	...
	replier.html('<p>Name: <a target="_blank" href=http://stackoverflow.com/users/' + answere.owner.user_id + ' >' +
												...
												'</a>' +
							'</p>' +
 							'<p>Reputation: ' + answere.owner.reputation + '</p>'
	);
